...
# EXPORTED VARIABLE
@export_range(0.0, 1.0, 0.01) var dominance_margin: float = 0.15
...
@onready var diagonal_overlay: TextureRect = $Base/DiagonalOverlay
...
func _update_joystick(touch_position: Vector2) -> void:
	...
	# ✅ Diagonal detection
	diagonal_overlay.visible = abs(output.x) > 0.3 and abs(output.y) > 0.3

	
	if use_input_actions:
		# Release actions first to clear previous state
		...
		# Press actions
		# Check if diagonal overlay is visible - if so, allow diagonal movement
		if diagonal_overlay.visible:
			# Diagonal movement - press both X and Y actions simultaneously
			if output.x < 0:
			...
			# Original behavior - favor dominant axis
			# Favor X-axis when both X and Y have input
			var dominance_margin := 0.15  # Adjust for sensitivity

			if abs(output.x) > abs(output.y) + dominance_margin:
				# X-axis dominates - release Y axis actions
				...
				if output.x < 0:
					...
					Input.action_press(action_right, output.x)
			elif abs(output.y) > abs(output.x) + dominance_margin:
				# Y-axis dominates - release X axis actions
				...
				# Neither axis dominates significantly - release all
				if Input.is_action_pressed(action_left):
					Input.action_release(action_left)
				if Input.is_action_pressed(action_right):
					Input.action_release(action_right)
...
func _reset():
	diagonal_overlay.visible = false
	is_pressed = false
	output = Vector2.ZERO
	_touch_index = -1
