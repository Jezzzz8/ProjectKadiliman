...
func get_current_day() -> int:
	return int(time / INGAME_TO_REAL_MINUTE_DURATION / MINUTES_PER_DAY)

func get_total_minutes() -> int:
	return int(time / INGAME_TO_REAL_MINUTE_DURATION)

func get_current_hour() -> int:
	return int((get_total_minutes() % MINUTES_PER_DAY) / MINUTES_PER_HOUR)

func get_current_minute() -> int:
	return int(get_total_minutes() % MINUTES_PER_HOUR)

func set_day_time(day: int, hour: int, minute: int) -> void:
	var total_minutes = (day * MINUTES_PER_DAY) + (hour * MINUTES_PER_HOUR) + minute
	time = total_minutes * INGAME_TO_REAL_MINUTE_DURATION
	_recalculate_time() # force update
